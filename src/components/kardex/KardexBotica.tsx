...
export function KardexBotica() {
  ...
  const { data: movements, isLoading } = useKardexMovements({ sistema: 'botica' });
  const { data: summary } = useKardexSummary();

  const boticaSummary = summary?.find(s => s.sistema === 'Botica');
  ...
  const handleRefresh = () => {
    ...
    toast({
      title: "Actualizado",
      description: "Los datos de Botica se han actualizado.",
    });
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Kardex Botica");
    XLSX.writeFile(wb, `kardex_botica_${format(new Date(), "yyyyMMdd_HHmm")}.xlsx`);
  ...
  // Agrupar movimientos por producto para vista de kardex individual
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleExportExcel} disabled={!filteredMovements?.length} className="bg-green-600 hover:bg-green-700">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{boticaSummary?.total_productos || 0}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{boticaSummary?.total_entradas || 0}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{boticaSummary?.total_salidas || 0}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{boticaSummary?.productos_bajo_stock || 0}</p>
              </div>
